...
- Gateway API: `GET /admin/enterprise/status` (JSON)
...
If an internal service is down, returns **HTTP 200** with `identityUnavailable` / `notificationUnavailable` and matching warnings — the page still renders with safe defaults for missing slices.
