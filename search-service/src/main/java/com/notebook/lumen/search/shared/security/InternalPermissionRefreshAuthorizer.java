...
@Component
public class InternalPermissionRefreshAuthorizer {
  public static final String REQUIRED_SCOPE = "internal:search:permission:write";

  private final SearchProperties properties;

  public InternalPermissionRefreshAuthorizer(SearchProperties properties) {
  ...
  public void authorize(String serviceAuthorization) {
    if (serviceAuthorization == null || serviceAuthorization.isBlank()) {
      throw new SearchException(
          HttpStatus.UNAUTHORIZED, "SEARCH_PERMISSION_ACCESS_DENIED", "Service JWT is required");
    }
    SearchProperties.TrustedService trustedService = properties.internal().trustedWorkspaceClient();
    ...
    try {
      ...
      if (e.insufficientScope()) {
        throw new SearchException(
            HttpStatus.FORBIDDEN, "SEARCH_PERMISSION_ACCESS_DENIED", e.getMessage());
      }
