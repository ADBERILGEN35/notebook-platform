...
class EmailNotificationServiceTest {
  ...
  private final EmailNotificationService service =
      new EmailNotificationService(repository, renderer, provider, properties(), auditService);
  ...
  void providerSuccessMarksNotificationSent() {
    ...
    when(repository.findDueNotifications(eq(EmailNotificationStatus.PENDING.name()), any()))
        .thenReturn(List.of(notification));
    when(provider.send(any()))
        .thenReturn(new EmailSendResult("noop", "provider-id", Instant.now()));
  ...
  void workerDisabledDoesNotFetchDueNotifications() {
    EmailNotificationService disabledService =
        new EmailNotificationService(
            repository, renderer, provider, disabledProperties(), auditService);

    disabledService.processDueNotifications();

    verify(repository, never()).findDueNotifications(any(), any());
  }
