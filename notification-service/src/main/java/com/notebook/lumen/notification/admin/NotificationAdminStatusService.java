...
public class NotificationAdminStatusService {
  ...
  public NotificationAdminStatusResponse build() {
    ...
    boolean redisChannelConfigured = distEnabled && channel != null && !channel.isBlank();
    var sse =
    ...
    var webhookBlock = new NotificationAdminStatusResponse.Webhook(email.webhooks().enabled());
  ...
  private static boolean emailProviderConfigured(
      String provider, NotificationProperties.Email email) {
    return switch (provider.toLowerCase()) {
      case "log", "noop" -> true;
      case "smtp" ->
          email.smtp() != null && email.smtp().host() != null && !email.smtp().host().isBlank();
      case "generic-http", "sendgrid" ->
          email.genericHttp() != null
