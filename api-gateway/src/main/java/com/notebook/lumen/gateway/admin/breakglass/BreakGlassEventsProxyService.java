...
@Service
public class BreakGlassEventsProxyService {
  static final String BREAK_GLASS_EVENTS_READ_SCOPE = "internal:admin:break-glass:events:read";
  static final String BREAK_GLASS_EVENTS_REVIEW_SCOPE = "internal:admin:break-glass:events:review";
  ...
  public BreakGlassEventsProxyService(
  ...
  public Mono<ResponseEntity<Object>> list(
      String status, String mode, String from, String to, int page, int size, String adminUserId, String requestId, String gatewayPath) {
    return Mono.defer(
        () -> {
          final String jwt;
          try {
            jwt = signer.sign(IDENTITY_AUDIENCE, BREAK_GLASS_EVENTS_READ_SCOPE);
          ...
          if (from != null && !from.isBlank()) ub.queryParam("from", from);
          if (to != null && !to.isBlank()) ub.queryParam("to", to);
  ...
  public Mono<ResponseEntity<Object>> detail(UUID id, String adminUserId, String requestId, String path) {
    return withAuthGet(baseUrl() + "/" + id, BREAK_GLASS_EVENTS_READ_SCOPE, adminUserId, requestId, path);
  }
    ...
    return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
        ...
        .body(
            new ErrorResponse(
                ...
                "Service JWT signing failed for break-glass proxy",
                path,
                requestId,
                null));
  }
  ...
  private String baseUrl() {
    ...
    return base + "/internal/admin/break-glass/events";
  }
}
