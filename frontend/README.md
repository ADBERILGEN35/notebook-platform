...
## Break-glass token rotation UI (Faz 94)

- Admin rotation governance page: `/app/admin/security/break-glass/rotation`
- Runtime flag: `FRONTEND_BREAK_GLASS_ROTATION_UI_ENABLED=false` (default)
- Permissions: `admin:break-glass:rotation:read` (list/detail), `admin:break-glass:rotation:manage` (acknowledge/verify/close, admin-write MFA)
- UI shows only short hash fingerprints (`fp:<prefix>`); the token value is never displayed, accepted, or generated by the page
...
## Conflict diff / suggested merge (Faz 66; extended in Faz 95)
...
- **Faz 95:** same-parent block reorders combined with disjoint remote edits join the safe
  suggested-merge set. Dialog now has `Moved blocks`, `Reordered blocks`, and `Move conflicts`
  sections (`data-testid="conflict-moved-blocks-summary"`, `conflict-reorder-summary`,
  `conflict-move-conflicts-summary`). No silent merge — user still confirms every apply.
- See `docs/note-conflict-diff-merge.md` and `docs/advanced-note-merge-rules.md`.
